...
public class AutoCrystal2 extends Module
{
    ...
    private Setting<Boolean> render2;
    ...
    public AutoCrystal2() {
        ...
        this.off = (Setting<Boolean>)this.register(new Setting("Second hand tap", (T)true, v -> this.setting.getValue() == Settings.BREAK));
        ...
        this.render2 = (Setting<Boolean>)this.register(new Setting("Silky rendering", (T)true));
        ...
        this.colorSync = (Setting<Boolean>)this.register(new Setting("ColorSync", (T)false, v -> this.setting.getValue() == Settings.RENDER));
        ...
        this.notek = (Setting<Boolean>)this.register(new Setting("The missing interface is not placed", (T)false));
        this.rotating = false;
        ...
        this.blockRenderSmooth = new BlockEasingRender(new BlockPos(0, 0, 0), 550.0f, 550.0f);
        AutoCrystal2.instance = this;
    }
    
    public static AutoCrystal2 getInstance() {
        if (AutoCrystal2.instance == null) {
            AutoCrystal2.instance = new AutoCrystal2();
        }
        return AutoCrystal2.instance;
    }
    ...
    public void onUpdate() {
        ...
        if (this.notek.getValue() && AutoCrystal2.mc.currentScreen instanceof GuiHopper) {
            this.placePos = null;
            return;
        }
    ...
    public void onToggle() {
        AutoCrystal2.brokenPos.clear();
        AutoCrystal2.placedPos.clear();
    ...
    public String getDisplayInfo() {
        ...
        if (AutoCrystal2.target != null) {
            return AutoCrystal2.target.getName();
        }
        return null;
    }
    ...
    public void onPacketSend(final PacketEvent.Send event) {
        ...
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (packet3 = (CPacketUseEntity)event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet3.getEntityFromWorld((World)AutoCrystal2.mc.world) instanceof EntityEnderCrystal) {
            pos = packet3.getEntityFromWorld((World)AutoCrystal2.mc.world).getPosition();
            if (this.removeAfterAttack.getValue()) {
                Objects.requireNonNull(packet3.getEntityFromWorld((World)AutoCrystal2.mc.world)).setDead();
                AutoCrystal2.mc.world.removeEntityFromWorld(packet3.entityId);
            }
        }
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (packet3 = (CPacketUseEntity)event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet3.getEntityFromWorld((World)AutoCrystal2.mc.world) instanceof EntityEnderCrystal) {
            final EntityEnderCrystal crystal = (EntityEnderCrystal)packet3.getEntityFromWorld((World)AutoCrystal2.mc.world);
            if (this.antiBlock.getValue() && EntityUtils.isCrystalAtFeet(crystal, this.range.getValue()) && pos != null) {
        ...
        if (!this.justRender.getValue() && this.switchTimer.passedMs(this.switchCooldown.getValue()) && this.explode.getValue() && this.instant.getValue() && event.getPacket() instanceof SPacketSpawnObject && (this.syncedCrystalPos == null || !this.syncedFeetPlace.getValue() || this.damageSync.getValue() == DamageSync.NONE)) {
            ...
            if (packet2.getType() == 51 && AutoCrystal2.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + this.predictOffset.getValue() <= MathUtil.square(this.breakRange.getValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || (this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue())) || (this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue())))) {
                ...
                if (this.predictFriendDmg.getValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (final EntityPlayer friend : AutoCrystal2.mc.world.playerEntities) {
                        if (friend != null && !AutoCrystal2.mc.player.equals((Object)friend) && friend.getDistanceSq(pos) <= MathUtil.square(this.range.getValue() + this.placeRange.getValue()) && Stay.friendManager.isFriend(friend)) {
                            if (DamageUtil.calculateDamage(pos, (Entity)friend) <= EntityUtils.getHealth((Entity)friend) + 0.5) {
                                continue;
                            }
                            return;
                        }
                ...
                if (AutoCrystal2.placedPos.contains(pos.down())) {
                    Label_0621: {
                        if (this.isRightThread() && this.superSafe.getValue()) {
                            ...
                            if ((selfDamage = DamageUtil.calculateDamage(pos, (Entity)AutoCrystal2.mc.player)) - 0.5 <= EntityUtils.getHealth((Entity)AutoCrystal2.mc.player)) {
                    ...
                    if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
                        selfDamage = DamageUtil.calculateDamage(pos, (Entity)AutoCrystal2.mc.player);
                    }
                    if (selfDamage + 0.5 < EntityUtils.getHealth((Entity)AutoCrystal2.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                        for (final EntityPlayer player : AutoCrystal2.mc.world.playerEntities) {
                            if (player.getDistanceSq(pos) <= MathUtil.square(this.range.getValue()) && EntityUtils.isValid((Entity)player, this.range.getValue() + this.breakRange.getValue()) && (!this.antiNaked.getValue() || !DamageUtil.isNaked(player))) {
                                final float damage;
                                if ((damage = DamageUtil.calculateDamage(pos, (Entity)player)) <= selfDamage && (damage <= this.minDamage.getValue() || DamageUtil.canTakeDamage(this.suicide.getValue())) && damage <= EntityUtils.getHealth((Entity)player)) {
                                    continue;
                                }
            ...
            for (final int id : packet4.getEntityIDs()) {
                final Entity entity = AutoCrystal2.mc.world.getEntityByID(id);
                if (entity instanceof EntityEnderCrystal) {
                    AutoCrystal2.brokenPos.remove(new BlockPos(entity.getPositionVector()).down());
                    AutoCrystal2.placedPos.remove(new BlockPos(entity.getPositionVector()).down());
                }
            }
        }
            ...
            if (packet5.getOpCode() == 35 && packet5.getEntity((World)AutoCrystal2.mc.world) instanceof EntityPlayer) {
                this.totemPops.put((EntityPlayer)packet5.getEntity((World)AutoCrystal2.mc.world), new Timer().reset());
            }
        }
        else {
            ...
            if (event.getPacket() instanceof SPacketSoundEffect && (packet6 = (SPacketSoundEffect)event.getPacket()).getCategory() == SoundCategory.BLOCKS && packet6.getSound() == SoundEvents.ENTITY_GENERIC_EXPLODE) {
                ...
                if (this.threadMode.getValue() == ThreadMode.SOUND && this.isRightThread() && AutoCrystal2.mc.player != null && AutoCrystal2.mc.player.getDistanceSq(pos) < MathUtil.square(this.soundPlayer.getValue())) {
    ...
    private boolean predictSlowBreak(final BlockPos pos) {
        return this.antiCommit.getValue() && AutoCrystal2.lowDmgPos.remove(pos) && this.shouldSlowBreak(false);
    }
    
    private boolean isRightThread() {
        return AutoCrystal2.mc.isCallingFromMinecraftThread() || (!Stay.eventManager.ticksOngoing() && !this.threadOngoing.get());
    }
    ...
    private void attackCrystalPredict(final int entityID, final BlockPos pos) {
        ...
        AutoCrystal2.mc.player.connection.sendPacket((Packet)attackPacket);
        if (this.breakSwing.getValue()) {
            AutoCrystal2.mc.player.connection.sendPacket((Packet)new CPacketAnimation(EnumHand.MAIN_HAND));
        }
    ...
    private void removePos(final BlockPos pos) {
        if (this.damageSync.getValue() == DamageSync.PLACE) {
            if (AutoCrystal2.placedPos.remove(pos)) {
                this.posConfirmed = true;
            }
        }
        else if (this.damageSync.getValue() == DamageSync.BREAK && AutoCrystal2.brokenPos.remove(pos)) {
            this.posConfirmed = true;
        }
    }
    ...
    public void onRender3D(final Render3DEvent event) {
        if (this.render2.getValue() && this.renderPos != null && this.render.getValue()) {
            synchronized (this.renderPos) {
                final Vec3d renders = this.blockRenderSmooth.getUpdate();
                if (renders == null) {
                    return;
                }
                RenderUtil.drawBoxESP(renders, ((boolean)this.colorSync.getValue()) ? ColorUtil.rainbow(ClickGui.getInstance().rainbowHue.getValue()) : new Color(this.red.getValue()), this.boxAlpha.getValue());
        ...
        else if (!this.render2.getValue() && (this.offHand || this.mainHand || this.switchMode.getValue() == Switch.CALC) && this.renderPos != null && this.render.getValue() && (this.box.getValue() || this.text.getValue() || this.outline.getValue())) {
            synchronized (this.renderPos) {
    ...
    public void onKeyInput(final InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoCrystal2.mc.currentScreen instanceof StayGui) && this.switchBind.getValue().getKey() == Keyboard.getEventKey()) {
            if (this.switchBack.getValue() && this.offhandSwitch.getValue() && this.offHand) {
                ...
                final Offhand module = ModuleManager.getModuleByClass(Offhand.class);
                if (module.isOff()) {
    ...
    private void postProcessBreak() {
        while (!this.packetUseEntities.isEmpty()) {
            ...
            AutoCrystal2.mc.player.connection.sendPacket((Packet)packet);
            if (this.breakSwing.getValue()) {
    ...
    private boolean check() {
        ...
        if ((!this.offHand && !this.mainHand && this.switchMode.getValue() == Switch.BREAKSLOT && !this.switching) || !DamageUtil.canBreakWeakness((EntityPlayer)AutoCrystal2.mc.player) || !this.switchTimer.passedMs(this.switchCooldown.getValue())) {
            this.renderPos = null;
            AutoCrystal2.target = null;
            return this.rotating = false;
        }
        if (this.mineSwitch.getValue() && Mouse.isButtonDown(0) && (this.switching || this.autoSwitch.getValue() == AutoSwitch.ALWAYS) && Mouse.isButtonDown(1) && AutoCrystal2.mc.player.getHeldItemMainhand().getItem() instanceof ItemPickaxe) {
            this.switchItem();
        }
        this.mapCrystals();
    ...
    private void mapCrystals() {
        ...
        for (final Entity entity : AutoCrystal2.mc.world.loadedEntityList) {
            if (!entity.isDead && entity instanceof EntityEnderCrystal) {
                ...
                if (this.syncedFeetPlace.getValue() && entity.getPosition().down().equals((Object)this.syncedCrystalPos) && this.damageSync.getValue() != DamageSync.NONE) {
                    ...
                    float selfDamage = -1.0f;
                    if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
                        selfDamage = DamageUtil.calculateDamage(entity, (Entity)AutoCrystal2.mc.player);
                    }
                    if (selfDamage + 0.5 < EntityUtils.getHealth((Entity)AutoCrystal2.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                        ...
                        for (final EntityPlayer player : AutoCrystal2.mc.world.playerEntities) {
                            ...
                            if (EntityUtils.isValid((Entity)player, this.range.getValue() + this.breakRange.getValue())) {
                                if (this.antiNaked.getValue() && DamageUtil.isNaked(player)) {
                                    continue;
                                }
                                final float damage;
                                if ((damage = DamageUtil.calculateDamage(entity, (Entity)player)) <= selfDamage && (damage <= this.minDamage.getValue() || DamageUtil.canTakeDamage(this.suicide.getValue())) && damage <= EntityUtils.getHealth((Entity)player)) {
                                    continue;
                                }
                                if (damage > maxDamage) {
                                    maxDamage = damage;
                                ...
                                if ((this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL) || !Stay.friendManager.isFriend(player.getName())) {
                                    continue;
                                }
                                final float damage;
                                if ((damage = DamageUtil.calculateDamage(entity, (Entity)player)) <= EntityUtils.getHealth((Entity)player) + 0.5) {
                                    continue;
                                }
        ...
        if (this.webAttack.getValue() && this.webPos != null) {
            if (AutoCrystal2.mc.player.getDistanceSq(this.webPos.up()) > MathUtil.square(this.breakRange.getValue())) {
                this.webPos = null;
            }
            else {
                for (final Entity entity : AutoCrystal2.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(this.webPos.up()))) {
                    if (!(entity instanceof EntityEnderCrystal)) {
                        continue;
                    }
        ...
        if (this.shouldSlowBreak(true) && maxDamage < this.minDamage.getValue() && (AutoCrystal2.target == null || EntityUtils.getHealth((Entity)AutoCrystal2.target) > this.facePlace.getValue() || (!this.breakTimer.passedMs(this.facePlaceSpeed.getValue()) && this.slowFaceBreak.getValue() && Mouse.isButtonDown(0) && this.holdFacePlace.getValue() && this.holdFaceBreak.getValue()))) {
            this.efficientTarget = null;
            return;
        }
        if (this.packets.getValue() == 1) {
    ...
    private void placeCrystal() {
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue()) && this.place.getValue() && (this.offHand || this.mainHand || this.switchMode.getValue() == Switch.CALC || (this.switchMode.getValue() == Switch.BREAKSLOT && this.switching))) {
            ...
            if (AutoCrystal2.target != null && this.placePos != null) {
                Stay.targetManager.updateTarget((EntityLivingBase)AutoCrystal2.target);
                if (!this.offHand && !this.mainHand && this.autoSwitch.getValue() != AutoSwitch.NONE && (this.currentDamage > this.minDamage.getValue() || (this.lethalSwitch.getValue() && EntityUtils.getHealth((Entity)AutoCrystal2.target) <= this.facePlace.getValue())) && !this.switchItem()) {
                    return;
                }
                if (this.currentDamage < this.minDamage.getValue() && this.limitFacePlace.getValue()) {
                    crystalLimit = 1;
                }
                if (this.currentDamage >= this.minMinDmg.getValue() && (this.offHand || this.mainHand || this.autoSwitch.getValue() != AutoSwitch.NONE) && (this.crystalCount < crystalLimit || (this.antiSurround.getValue() && this.lastPos != null && this.lastPos.equals((Object)this.placePos))) && (this.currentDamage > this.minDamage.getValue() || this.minDmgCount < crystalLimit) && this.currentDamage >= 1.0 && (DamageUtil.isArmorLow(AutoCrystal2.target, this.minArmor.getValue()) || EntityUtils.getHealth((Entity)AutoCrystal2.target) <= this.facePlace.getValue() || this.currentDamage > this.minDamage.getValue() || this.shouldHoldFacePlace())) {
                    final float damageOffset = (this.damageSync.getValue() == DamageSync.BREAK) ? (this.dropOff.getValue() - 5.0f) : 0.0f;
                    boolean syncflag = false;
                    if (this.syncedFeetPlace.getValue() && this.placePos.equals((Object)this.lastPos) && this.isEligableForFeetSync(AutoCrystal2.target, this.placePos) && !this.syncTimer.passedMs(this.damageSyncTime.getValue()) && AutoCrystal2.target.equals((Object)this.currentSyncTarget) && AutoCrystal2.target.getPosition().equals((Object)this.syncedPlayerPos) && this.damageSync.getValue() != DamageSync.NONE) {
                    ...
                    if (syncflag || this.currentDamage - damageOffset > this.lastDamage || this.syncTimer.passedMs(this.damageSyncTime.getValue()) || this.damageSync.getValue() == DamageSync.NONE) {
                        ...
                        this.blockRenderSmooth.updatePos(this.renderPos);
                        this.renderDamage = this.currentDamage;
                        if (this.switchItem()) {
                            this.currentSyncTarget = AutoCrystal2.target;
                            this.syncedPlayerPos = AutoCrystal2.target.getPosition();
                            if (this.foundDoublePop) {
                                this.totemPops.put(AutoCrystal2.target, new Timer().reset());
                            }
                            ...
                            if (this.addTolowDmg || (this.actualSlowBreak.getValue() && this.currentDamage < this.minDamage.getValue())) {
                                AutoCrystal2.lowDmgPos.add(this.placePos);
                            }
                            AutoCrystal2.placedPos.add(this.placePos);
    ...
    private void calculateDamage(final EntityPlayer targettedPlayer) {
        ...
        final Block web;
        if (this.webAttack.getValue() && targettedPlayer != null && (web = AutoCrystal2.mc.world.getBlockState(playerPos = new BlockPos(targettedPlayer.getPositionVector())).getBlock()) == Blocks.WEB) {
            setToAir = playerPos;
        ...
        for (final BlockPos pos : BlockUtil.possiblePlacePositions(this.placeRange.getValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            if (!BlockUtil.rayTracePlaceCheck(pos, (this.raytrace.getValue() == Raytrace.PLACE || this.raytrace.getValue() == Raytrace.FULL) && AutoCrystal2.mc.player.getDistanceSq(pos) > MathUtil.square(this.placetrace.getValue()), 1.0f)) {
                continue;
            }
            float selfDamage = -1.0f;
            if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
            ...
            if (selfDamage + 0.5 >= EntityUtils.getHealth((Entity)AutoCrystal2.mc.player)) {
                continue;
            }
            ...
            if (targettedPlayer != null) {
                ...
                if (this.calcEvenIfNoDamage.getValue() && (this.antiFriendPop.getValue() == AntiFriendPop.ALL || this.antiFriendPop.getValue() == AntiFriendPop.PLACE)) {
                    ...
                    for (final EntityPlayer friend : AutoCrystal2.mc.world.playerEntities) {
                        if (friend != null && !AutoCrystal2.mc.player.equals((Object)friend) && friend.getDistanceSq(pos) <= MathUtil.square(this.range.getValue() + this.placeRange.getValue()) && Stay.friendManager.isFriend(friend)) {
                            ...
                            if ((friendDamage = DamageUtil.calculateDamage(pos, (Entity)friend)) <= EntityUtils.getHealth((Entity)friend) + 0.5) {
                                continue;
                            }
                            friendPop = true;
                            break;
                ...
                if (this.isDoublePoppable(targettedPlayer, playerDamage) && (currentPos == null || targettedPlayer.getDistanceSq(pos) < targettedPlayer.getDistanceSq(currentPos))) {
                    ...
                    if (playerDamage <= selfDamage && (playerDamage <= this.minDamage.getValue() || DamageUtil.canTakeDamage(this.suicide.getValue())) && playerDamage <= EntityUtils.getHealth((Entity)targettedPlayer)) {
                        continue;
                    }
                    maxDamage = playerDamage;
                    currentTarget = targettedPlayer;
                ...
                for (final EntityPlayer player : AutoCrystal2.mc.world.playerEntities) {
                    if (EntityUtils.isValid((Entity)player, this.placeRange.getValue() + this.range.getValue())) {
                        if (this.antiNaked.getValue() && DamageUtil.isNaked(player)) {
                            continue;
                        }
                        ...
                        if (this.doublePopOnDamage.getValue() && this.isDoublePoppable(player, playerDamage2) && (currentPos == null || player.getDistanceSq(pos) < player.getDistanceSq(currentPos))) {
                            ...
                            if (playerDamage2 <= selfDamage && (playerDamage2 <= this.minDamage.getValue() || DamageUtil.canTakeDamage(this.suicide.getValue())) && playerDamage2 <= EntityUtils.getHealth((Entity)player)) {
                                continue;
                            }
                            maxDamage = playerDamage2;
                            currentTarget = player;
                        ...
                        if ((this.antiFriendPop.getValue() != AntiFriendPop.ALL && this.antiFriendPop.getValue() != AntiFriendPop.PLACE) || player == null || player.getDistanceSq(pos) > MathUtil.square(this.range.getValue() + this.placeRange.getValue()) || !Stay.friendManager.isFriend(player)) {
                            continue;
                        }
                        final float friendDamage2;
                        if ((friendDamage2 = DamageUtil.calculateDamage(pos, (Entity)player)) <= EntityUtils.getHealth((Entity)player) + 0.5) {
                            continue;
                        }
        ...
        AutoCrystal2.target = currentTarget;
    ...
    private EntityPlayer getTarget(final boolean unsafe) {
        ...
        EntityPlayer currentTarget = null;
        for (final EntityPlayer player : AutoCrystal2.mc.world.playerEntities) {
            if (!EntityUtils.isntValid((Entity)player, this.placeRange.getValue() + this.range.getValue()) && (!this.antiNaked.getValue() || !DamageUtil.isNaked(player))) {
                if (unsafe && EntityUtils.isSafe((Entity)player)) {
                    continue;
                }
                ...
                if (currentTarget == null) {
                    ...
                    if (AutoCrystal2.mc.player.getDistanceSq((Entity)player) >= AutoCrystal2.mc.player.getDistanceSq((Entity)currentTarget)) {
                        continue;
                    }
                    currentTarget = player;
                }
        ...
        if (this.predictPos.getValue() && currentTarget != null) {
            ...
            final EntityOtherPlayerMP newTarget = new EntityOtherPlayerMP((World)AutoCrystal2.mc.world, profile);
            ...
            newTarget.setHealth(EntityUtils.getHealth((Entity)currentTarget));
    ...
    private void attackEntity(final Entity entity) {
        if (entity != null) {
            if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE && this.rotateFirst.getValue() && this.rotate.getValue() != Rotate.OFF) {
                ...
                AutoCrystal2.brokenPos.add(new BlockPos(entity.getPositionVector()).down());
            }
        }
    }
    ...
    private void doFakeSwing() {
        if (this.fakeSwing.getValue()) {
            EntityUtils.swingArmNoPacket(EnumHand.MAIN_HAND, (EntityLivingBase)AutoCrystal2.mc.player);
        }
    }
    ...
    private void manualBreaker() {
        if (this.rotate.getValue() != Rotate.OFF && this.eventMode.getValue() != 2 && this.rotating) {
            if (this.didRotation) {
                AutoCrystal2.mc.player.rotationPitch += (float)4.0E-4;
                this.didRotation = false;
            }
            else {
                AutoCrystal2.mc.player.rotationPitch -= (float)4.0E-4;
        ...
        if ((this.offHand || this.mainHand) && this.manual.getValue() && this.manualTimer.passedMs(this.manualBreak.getValue()) && Mouse.isButtonDown(1) && AutoCrystal2.mc.player.getHeldItemOffhand().getItem() != Items.GOLDEN_APPLE && AutoCrystal2.mc.player.inventory.getCurrentItem().getItem() != Items.GOLDEN_APPLE && AutoCrystal2.mc.player.inventory.getCurrentItem().getItem() != Items.BOW && AutoCrystal2.mc.player.inventory.getCurrentItem().getItem() != Items.EXPERIENCE_BOTTLE && (result = AutoCrystal2.mc.objectMouseOver) != null) {
            switch (result.typeOfHit) {
                ...
                case BLOCK: {
                    final BlockPos mousePos = AutoCrystal2.mc.objectMouseOver.getBlockPos().up();
                    for (final Entity target : AutoCrystal2.mc.world.getEntitiesWithinAABBExcludingEntity((Entity)null, new AxisAlignedBB(mousePos))) {
    ...
    private void rotateTo(final Entity entity) {
        switch (this.rotate.getValue()) {
            ...
            case BREAK:
            case ALL: {
                final float[] angle = MathUtil.calcAngle(AutoCrystal2.mc.player.getPositionEyes(AutoCrystal2.mc.getRenderPartialTicks()), entity.getPositionVector());
    ...
    private void rotateToPos(final BlockPos pos) {
        switch (this.rotate.getValue()) {
            ...
            case PLACE:
            case ALL: {
                final float[] angle = MathUtil.calcAngle(AutoCrystal2.mc.player.getPositionEyes(AutoCrystal2.mc.getRenderPartialTicks()), new Vec3d((double)(pos.getX() + 0.5f), (double)(pos.getY() - 0.5f), (double)(pos.getZ() + 0.5f)));
                if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE) {
                    Stay.rotationManager.setPlayerRotations(angle[0], angle[1]);
                    break;
                }
    ...
    private boolean isDoublePoppable(final EntityPlayer player, final float damage) {
        final float health;
        if (this.doublePop.getValue() && (health = EntityUtils.getHealth((Entity)player)) <= this.popHealth.getValue() && damage > health + 0.5 && damage <= this.popDamage.getValue()) {
    ...
    private boolean isValid(final Entity entity) {
        return entity != null && AutoCrystal2.mc.player.getDistanceSq(entity) <= MathUtil.square(this.breakRange.getValue()) && (this.raytrace.getValue() == Raytrace.NONE || this.raytrace.getValue() == Raytrace.PLACE || AutoCrystal2.mc.player.canEntityBeSeen(entity) || (!AutoCrystal2.mc.player.canEntityBeSeen(entity) && AutoCrystal2.mc.player.getDistanceSq(entity) <= MathUtil.square(this.breaktrace.getValue())));
    }
    ...
    static {
        AutoCrystal2.target = null;
        AutoCrystal2.lowDmgPos = (Set<BlockPos>)new ConcurrentSet();
        AutoCrystal2.placedPos = new HashSet<BlockPos>();
        AutoCrystal2.brokenPos = new HashSet<BlockPos>();
    }
    ...
    private static class RAutoCrystal implements Runnable
    {
        ...
        public static RAutoCrystal getInstance(final AutoCrystal2 autoCrystal2) {
        ...
        public void run() {
            if (this.autoCrystal2.threadMode.getValue() == ThreadMode.WHILE) {
                while (this.autoCrystal2.isOn() && this.autoCrystal2.threadMode.getValue() == ThreadMode.WHILE) {
                    ...
                    try {
                        Thread.sleep(this.autoCrystal2.threadDelay.getValue());
                    }
