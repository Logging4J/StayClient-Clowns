...
public class AutoCrystal extends Module
{
    ...
    public AutoCrystal() {
        ...
        this.predicts = (Setting<Boolean>)this.register(new Setting("Inhibit", (T)true));
        ...
        this.ecmeplace = (Setting<Boolean>)this.register(new Setting("1.13 Place", (T)false));
        ...
        this.removeAttack = (Setting<Boolean>)this.register(new Setting("EntityRemove", (T)false));
    ...
    public void onPacketSend(final PacketEvent.Send event) {
        ...
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity) {
            ...
            if (this.removeAttack.getValue()) {
                packet2.getEntityFromWorld((World)AutoCrystal.mc.world).setDead();
                AutoCrystal.mc.world.removeEntityFromWorld(packet2.entityId);
            }
        }
    }
    ...
    public void onCrystal() {
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue().longValue()) && this.place.getValue()) {
            ...
            Stay.targetManager.updateTarget(this.realTarget);
    ...
    public void onPacketReceive(final PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof SPacketSpawnObject && (packet = (SPacketSpawnObject)event.getPacket()).getType() == 51 && this.predicts.getValue() && this.preditTimer.passedMs(this.attackFactor.getValue()) && this.explode.getValue() && this.packetBreak.getValue() && this.target != null) {
    ...
    private boolean isPredicting(final SPacketSpawnObject packet) {
        ...
        if (EntityUtils.isInHole((Entity)AutoCrystal.mc.player) && targetDmg >= 1.0) {
            return true;
        }
        ...
        return (targetDmg >= this.breakMinDmg.getValue() && selfDmg <= this.breakMaxSelfDamage.getValue()) || (EntityUtils.isInHole((Entity)this.target) && this.target.getHealth() + this.target.getAbsorptionAmount() <= this.facePlace.getValue());
    }
    ...
    EntityPlayer getTarget() {
        ...
        for (final EntityPlayer entity : AutoCrystal.mc.world.playerEntities) {
            if (AutoCrystal.mc.player != null && !AutoCrystal.mc.player.isDead && !entity.isDead && entity != AutoCrystal.mc.player && !Stay.friendManager.isFriend(entity.getName())) {
                ...
                if (EntityUtils.isInHole((Entity)entity) && entity.getAbsorptionAmount() + entity.getHealth() > this.facePlace.getValue() && !this.armorTarget && this.minDamage.getValue() > 2.2f) {
                    continue;
                }
