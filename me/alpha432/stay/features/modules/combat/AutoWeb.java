...
public class AutoWeb extends Module
{
    ...
    public void onEnable() {
        ...
        this.startPos = EntityUtils.getRoundedBlockPos((Entity)AutoWeb.mc.player);
    ...
    private boolean check() {
        ...
        if (this.disable.getValue() && !this.startPos.equals((Object)EntityUtils.getRoundedBlockPos((Entity)AutoWeb.mc.player))) {
            this.disable();
            return true;
        }
        if (obbySlot == -1) {
    ...
    private EntityPlayer getTarget(final double range) {
        ...
        for (final EntityPlayer player : AutoWeb.mc.world.playerEntities) {
            if (!EntityUtils.isntValid((Entity)player, range) && !player.isInWeb) {
                if (Stay.speedManager.getPlayerSpeed(player) > 30.0) {
                    continue;
                }
                if (target == null) {
                    target = player;
