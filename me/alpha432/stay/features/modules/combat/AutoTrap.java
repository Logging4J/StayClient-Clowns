...
public class AutoTrap extends Module
{
    ...
    public void onEnable() {
        ...
        this.startPos = EntityUtils.getRoundedBlockPos((Entity)AutoTrap.mc.player);
    ...
    private void doStaticTrap() {
        final List<Vec3d> placeTargets = EntityUtils.targets(this.target.getPositionVector(), this.antiScaffold.getValue(), this.antiStep.getValue(), false, false, false, this.raytrace.getValue());
    ...
    private boolean check() {
        ...
        if (!this.startPos.equals((Object)EntityUtils.getRoundedBlockPos((Entity)AutoTrap.mc.player))) {
            this.disable();
            return true;
        }
    ...
    private EntityPlayer getTarget(final double range, final boolean trapped) {
        ...
        for (final EntityPlayer player : AutoTrap.mc.world.playerEntities) {
            if (!EntityUtils.isntValid((Entity)player, range) && (!trapped || !EntityUtils.isTrapped(player, this.antiScaffold.getValue(), this.antiStep.getValue(), false, false, false))) {
