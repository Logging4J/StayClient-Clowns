...
public class ModuleButton extends Button
{
    ...
    public ModuleButton(final Module module) {
        ...
        this.logo = new ResourceLocation("textures/stayss.png");
        ...
        if (!this.module.getSettings().isEmpty()) {
            for (final Setting<?> setting : this.module.getSettings()) {
                ...
                if (setting.isNumberSetting() && setting.hasRestriction()) {
                    ...
                    newItems.add((Item)new EnumButton((Setting)setting));
                }
            }
        }
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        if (!this.items.isEmpty()) {
            if (HUD.getInstance().magenDavid.getValue()) {
                ...
                drawCompleteImage(this.x - 1.5f + this.width - 7.4f, this.y - 2.2f - StayGui.getClickGui().getTextOffset(), 8, 8);
            }
            if (this.subOpen) {
                float height = 1.0f;
