...
public class Component extends Feature
{
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        Gui.drawRect(this.x - 2, this.y - 1, this.x + this.width + 2, this.y + this.height - 6, ((boolean)ClickGui.getInstance().rainbow.getValue()) ? ColorUtil.rainbow(ClickGui.getInstance().rainbowHue.getValue()).getRGB() : color);
        if (this.open) {
            RenderUtil.drawRect((float)this.x, this.y + 12.0f, (float)(this.x + this.width), this.y + this.height + totalItemHeight, new Color(0, 0, 0, 50).getRGB());
        }
        assert Stay.textManager != null;
        Stay.textManager.drawStringWithShadow(this.getName(), this.x + 3.0f, this.y - 4.0f - StayGui.getClickGui().getTextOffset(), -1);
        if (this.open) {
    ...
    public Component setWidth(final int width) {
        this.width = width;
        return this;
    }
