...
public class GuiUtil
{
    ...
    public static void drawString(final String paramString, final int paramInt1, final int paramInt2, final int paramInt3) {
        final ModuleManager moduleManager = Stay.moduleManager;
        if (ModuleManager.getModuleByName("CustomFont").isEnabled()) {
            Stay.fontRenderer.drawStringWithShadow(paramString, (float)paramInt1, (float)paramInt2, paramInt3);
        }
        else {
    ...
    public static int getHeight() {
        final ModuleManager moduleManager = Stay.moduleManager;
        return ModuleManager.getModuleByName("CustomFont").isEnabled() ? Stay.fontRenderer.getFontHeight() : Stay.fontRenderer.getFontHeight();
    }
    ...
    public static void drawCenteredString(final String paramString, final int paramInt1, final int paramInt2, final int paramInt3) {
        final ModuleManager moduleManager = Stay.moduleManager;
        if (ModuleManager.getModuleByName("CustomFont").isEnabled()) {
            Stay.fontRenderer.drawStringWithShadow(paramString, (float)(paramInt1 - Stay.fontRenderer.getStringWidth(paramString) / 2), (float)paramInt2, paramInt3);
        }
        else {
            Minecraft.getMinecraft().fontRenderer.drawStringWithShadow(paramString, (float)(paramInt1 - Stay.fontRenderer.getStringWidth(paramString) / 2), (float)paramInt2, paramInt3);
        }
    }
}
