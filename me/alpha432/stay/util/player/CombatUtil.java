...
public class CombatUtil
{
    ...
    public static EntityEnderCrystal getClosestValidCrystal(final EntityPlayer player, final double maxSelfDamage, final double maxFriendDamage, final double minDamage, final double breakRange, final double wallsRange, final double friendRange, final boolean rayTrace, final boolean antiSuicide, final boolean antiFriendPop) {
        if (player == null) {
            return null;
        }
        final List<EntityEnderCrystal> crystals = (List<EntityEnderCrystal>)CombatUtil.mc.world.loadedEntityList.stream().filter(entity -> entity instanceof EntityEnderCrystal).filter(entity -> CombatUtil.mc.player.getDistanceSq(entity) <= breakRange * breakRange).sorted(Comparator.comparingDouble(entity -> CombatUtil.mc.player.getDistanceSq(entity))).map(entity -> entity).collect(Collectors.toList());
        for (final EntityEnderCrystal crystal : crystals) {
            if (rayTrace && !rayTraceRangeCheck((Entity)crystal, wallsRange)) {
                continue;
            }
            if (calculateDamage((Entity)crystal, (Entity)player) < minDamage) {
                continue;
            }
            if (!checkSelf(crystal, maxSelfDamage, antiSuicide)) {
                continue;
            }
            if (!checkFriends(crystal, maxFriendDamage, friendRange, antiFriendPop)) {
                continue;
            }
            return crystal;
        }
